...
def play_audio_feedback(text, filepath, last_audio_end_time, color, last_filepath, green_queue, red_cooldown=2.0, green_cooldown = 5.0):
    ...
    def ensure_audio_file_exists(path):
        if text and not os.path.exists(path):
            tts = gTTS(text=text, lang='en')
            tts.save(path)

    # If the feedback is red, play immediately if not on cooldown. Skip all green feedback
    if is_red:
        ensure_audio_file_exists(filepath)
        ...
        return last_audio_end_time, last_filepath, green_queue
    ...
    if is_green:
        ...
        if filepath not in green_queue:
            green_queue.append(filepath)
    ...
    if not pygame.mixer.music.get_busy() and current_time >= last_audio_end_time:
            if green_queue:
                next_filepath = green_queue.popleft()
                ensure_audio_file_exists(next_filepath)
